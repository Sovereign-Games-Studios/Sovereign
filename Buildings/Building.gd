...
func initialize(building_name, start_position):
	...
	if(self.building_definition.sprite_override):
		$Sprite3D.texture = load("res://Resources/Buildings/Images/"+self.building_definition.sprite_override+".png")
	else:
		$Sprite3D.texture = load("res://Resources/Buildings/Images/"+building_name+".png")		
