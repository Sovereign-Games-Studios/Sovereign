...
func _item_check(npc: NPC, new_item: Resource):
	for equipment in npc.current_equipment:
		# If the item we are looking at doesn't match the slot, skip it.
		if equipment != new_item.type:
			continue
		if npc.current_equipment[equipment] != null:
			var current_item = npc.current_equipment[equipment]
			var new_item_stat
